...
os.chdir("/AUTOMATION/ConfigAudit")

filename = input("Please specify a file name to save report : ")

workbook = xlsxwriter.Workbook('/AUTOMATION/ConfigAudit/' + filename + '.xlsx')
...
device_type = 'juniper'
devicelist = input("Enter the device list filename : " )
#print(devicelist)
devices = open(devicelist + '.txt')
row = 0
...
for IP in devices:
    ...
    try:
        ...
        cprint("Collecting data from " + IP, 'green')
        ...
        cprint("Printing data in Excell sheet " + IP, 'yellow')
        for i in tqdm(range(3)):
            time.sleep(0.2)
